...
class ExpenseReportForCashier(models.Model):
    _name = 'expense.report.for.cashier'
    ...
    def init(self):
        tools.drop_view_if_exists(self.env.cr, 'expense_report_for_cashier')
        self.env.cr.execute(""" 
            CREATE OR REPLACE VIEW expense_report_for_cashier AS (
                SELECT
