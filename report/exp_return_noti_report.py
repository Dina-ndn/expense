...
class ExpReturnNotiReport(models.Model):
    _name = 'exp.return.noti.report'
    _description = 'Waaneiza Expense Return Notification Report'
    _auto = False
    ...
    def init(self):
        tools.drop_view_if_exists(self.env.cr, 'exp_return_noti_report')
        self.env.cr.execute(""" 
            CREATE OR REPLACE VIEW exp_return_noti_report AS (
                SELECT
